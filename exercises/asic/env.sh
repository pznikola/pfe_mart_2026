...
if [[ -n "${BASH_SOURCE[0]}" ]]; then
    export FIFO_ROOT=$(realpath $(dirname "${BASH_SOURCE[0]}"))
else
    export FIFO_ROOT=$(pwd)
fi
echo "[INFO][ENV] FIFO root: $FIFO_ROOT"
...
if [[ -d "${FIFO_ROOT}/technology" ]]; then

    echo "[INFO][ENV] Init tech from ETHZ DZ cockpit"
    export PDK_ROOT="$FIFO_ROOT/technology"
    export KLAYOUT_PATH="$FIFO_ROOT/klayout/.klayout"
    ...
    export PDK_DIR_LEF_BOND="$FIFO_ROOT/ihp13/bondpad/lef"
    export PDK_DIR_GDS_CELLS="$PDK_ROOT/gds"
    export PDK_DIR_GDS_SRAMS="$PDK_ROOT/gds"
    export PDK_DIR_GDS_IOS="$PDK_ROOT/gds"
    export PDK_DIR_GDS_BOND="$FIFO_ROOT/ihp13/bondpad/gds"

elif [[ -d "${FIFO_ROOT}/ihp13/pdk" ]]; then

    echo "[INFO][ENV] Init tech from Github PDK"
    export PDK_ROOT="$FIFO_ROOT/ihp13/pdk"
    ...
    if [ ! -f ${FIFO_ROOT}/ihp13/pdk.patched ]; then
        git -C ${PDK_ROOT} apply ../patches/0001-Filling-improvements.patch
        touch ${FIFO_ROOT}/ihp13/pdk.patched
